...
const TodoItem = ({ atom, remove }: TodoItemProps) => {
  ...
  const toggleCompleted = () => {
    setItem((props: any) => (console.log(props), { ...props, completed: !props.completed }))
  }
...
const Filter = () => {
  ...
  return (
    <Radio.Group onChange={(e: any) => setFilter(e.target.value)} value={filter}>
...
const Filtered = (props: FilteredType) => {
  ...
  const transitions = useTransition(todos, {
    keys: (todo: any) => todo.toString(),
    from: { opacity: 0, height: 0 },
    enter: { opacity: 1, height: 40 },
    leave: { opacity: 0, height: 0 },
  })
  return transitions((style: any, atom: any) => (
